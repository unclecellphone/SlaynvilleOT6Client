...
function addKeyCombo(keyCombo, keySettings, focus)
  ...
  if not hotkeyLabel then
    ...
    if keySettings then
      ...
      hotkeyLabel.useType = HOTKEY_MANAGER_USEWITH --tonumber(keySettings.useType)
...
function doKeyCombo(keyCombo, repeated)
  ...
  --hardcode useWith useless use (like food)
  if not hotKey.useType == HOTKEY_MANAGER_USE then
    hotKey.useType = HOTKEY_MANAGER_USEWITH
  end
...
function updateHotkeyForm(reset)
  ...
  if currentHotkeyLabel then
    if not currentHotkeyLabel.useType == HOTKEY_MANAGER_USE then
      currentHotkeyLabel.useType = HOTKEY_MANAGER_USEWITH
    end
  end
